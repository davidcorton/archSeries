#' Simulate chronological distributions for subsets of a data table.
#' 
...
#' 
#' @param data Data table (or object that can be coerced to one) with, minimally, two numeric columns called Start and End and a factor
...
#' @param weight Numeric vector: the weight to be applied to each row in `data` (and to its counterpart in the dummy set, where relevant), 
#'      or a constant weight to be applied to all. Defaults to 1.
#' @param comp.field Character: name of column to be used to subset 'data'. Defaults to "group".
#' @param comp.values Vector with length at least 2, specifying values of 'comp.field' to compare. Defaults to NULL, in which case all 
...
#' @param comp.fun The function to apply to each subset of 'data'. Defaults to date.simulate.
...
comp.simulate <- function(data, probs=1, weight=1, comp.field="group", comp.values=NULL, context.fields=c("SITE_C"), UoA=NULL, 
                          comp.fun=date.simulate, quant.list=c(0.025, 0.25, 0.5, 0.75, 0.975), start.date=0, end.date=2000, bin.width=100, 
                          reps=100, RoC=FALSE) {
    ...
    for(i in 1:length(comp.values)) {
        y <- comp.fun(data=data[get(comp.field)==comp.values[i], ], probs=probs, weight=data[get(comp.field)==comp.values[i], weight], 
                      filter.field=filter.field, filter.values=filter.values, quant.list=quant.list, UoA=UoA, context.field=context.fields, 
